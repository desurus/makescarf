...
class Widget extends \Pure\Widget {
	...
	public function widget() {
		...
		if(false != $menu) {
			...
			//Set the menu depth level and is_parent params
			foreach($menu_items as $key => $menu_item) {
				if($menu_item->menu_item_parent) {
					//Find the menu item with this ID
					foreach($menu_items as $j => $_menu_item) {
						if($_menu_item->ID == $menu_item->menu_item_parent) {
							$_menu_item->is_parent = true;
							$menu_items[$j] = $_menu_item;	
						}
					}
					$menu_item->depth = 2;
				} else {
